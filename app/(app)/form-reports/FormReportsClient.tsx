...
export default function FormReportsClient() {
  ...
  function printFlaggedQueueReport() {
    ...
    const generatedBy =
      (currentUserId ? nameById[currentUserId] : null) || "Unknown user";
    const filterSummary = [
      `Status: ${queueStatusFilter}`,
      `Asset: ${queueAssetFilter}`,
      `Category: ${queueCategoryFilter}`,
...
<head>
  ...
  <style>
    ...
    .filters { margin-bottom: 14px; font-size: 12px; color: #333; border: 1px solid #bbb; border-radius: 6px; padding: 8px; }
    ...
    .signatures { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 18px; margin-top: 20px; font-size: 12px; }
    .sig-line { border-top: 1px solid #222; margin-top: 28px; padding-top: 4px; }
    .footer { position: fixed; bottom: 0; left: 0; right: 0; font-size: 10px; color: #333; display: flex; justify-content: space-between; padding: 4px 10mm; }
...
<body>
  <div class="header">
    ...
    <div style="font-size: 11px; text-align: right;">
      ...
      <div>Generated: ${escapeHtml(openedAt)}</div>
    </div>
  </div>
  <h1>Flagged Queue Report</h1>
  <div class="meta">Rows: ${flaggedQueueRows.length}</div>
  ...
  <div class="signatures">
    ...
    <div><div class="sig-line">Reviewed by</div></div>
    <div><div class="sig-line">Date</div></div>
  </div>
  <div class="footer">
    <div>Outdoor Independence LLC</div>
    <div class="page"></div>
  </div>
</body>
</html>`;
